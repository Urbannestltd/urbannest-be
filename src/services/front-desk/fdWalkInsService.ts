...
export class FdWalkInsService {
  ...
  private async assertFdOwnsVisit(fdId: string, visitId: string) {
    const visit = await prisma.visitorInvite.findUnique({
      ...
      include: {
        ...
        registeredByFd: { select: { userFullName: true } },
      },
    });
  ...
  public async registerWalkIn(
    ...
    // Validate unit belongs to a front-desk-managed property
    const unit = await prisma.unit.findUnique({
      where: { id: data.unitId },
      include: {
        property: {
          select: { id: true, name: true, frontDeskId: true, type: true },
        },
      },
    });
    if (!unit) throw new NotFoundError("Unit not found");
    if (unit.property.frontDeskId !== fdId) {
    ...
    const fd = await prisma.user.findUnique({
      where: { userId: fdId },
  ...
  public async checkOut(fdId: string, visitId: string): Promise<void> {
    const visit = await this.assertFdOwnsVisit(fdId, visitId);
  ...
  public async listWalkIns(
    ...
    const managedProperties = await prisma.property.findMany({
      where: { frontDeskId: fdId, isDeleted: false },
      select: { id: true },
    });
